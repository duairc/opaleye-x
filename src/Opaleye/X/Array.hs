...
------------------------------------------------------------------------------
instance
    ( Default QueryRunner (Column (O.PGArray ps)) [Maybe a]
    , ps ~ DistributeNullableInner p
    )
  =>
    Default (R [] QueryRunner) (PGMaybe (Column (O.PGArray p))) (Maybe a)
  where
    def = R $ lmap (\(PGMaybe a) -> a) def
